...
class KalshiHttpClient(KalshiBaseClient):
    ...
    def delete(self, path: str, params: Dict[str, Any] = {}) -> Any:
        ...
        self.rate_limit()
        response = requests.delete(self.host + path, headers=self.request_headers("DELETE", path), params=params)
        self.raise_if_bad_response(response)
        return response.json()
...
class KalshiWebSocketClient(KalshiBaseClient):
    ...
    async def subscribe_to_tickers(self):
        ...
        subscription_message = {"id": self.message_id, "cmd": "subscribe", "params": {"channels": ["ticker"]}}
    ...
    async def on_message(self, message):
        ...
        with open("ticker_updates.json", "a") as f:
            f.write(message + "\n")

    async def on_error(self, error):
        """Callback for handling errors."""
        with open("websocket_errors.json", "a") as f:
            f.write(str(error) + "\n")

    async def on_close(self, close_status_code, close_msg):
        """Callback when WebSocket connection is closed."""
        with open("websocket_closes.json", "a") as f:
            f.write(f"Connection closed with status code {close_status_code}: {close_msg}\n")
